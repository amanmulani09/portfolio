...
const knowledge: { pattern: RegExp; reply: () => Answer }[] = [
  ...
  { pattern: /\b(resume|cv|download)\b/, reply: () => answer("resume", "Aman’s résumé is available on request. Open the Google Drive link in a new tab, request access, and include a reason.", [link("Request résumé", profile.resume)]) },
  ...
  { pattern: /\b(experience|career|background|years|history|companies|roles|worked)\b/, reply: () => answer("experience", `Aman has been building software professionally since August 2022. ${experiences.map((item) => `${item.company}: ${item.role}, ${item.period}.`).join(" ")}`, [link("View timeline", "/#experience"), link("Request résumé", profile.resume)]) },
  ...
  { pattern: /\b(skills?|stack|technologies|toolkit|tools|languages)\b/, reply: () => answer("skills", skillGroups.map((group) => `${group.label}: ${group.skills.join(", ")}.`).join(" "), [link("View skills", "/#about"), link("Request résumé", profile.resume)]) },
